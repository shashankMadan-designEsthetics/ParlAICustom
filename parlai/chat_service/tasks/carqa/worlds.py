...
# ---------- Chatbot demo ---------- #
class CarBotChatOnboardWorld(OnboardWorld):
    """
    Example Car onboarding world for Chatbot Model.
    """

    @staticmethod
    def generate_world(opt, agents):
        return CarBotChatOnboardWorld(opt=opt, agent=agents[0])

    def parley(self):
        self.episodeDone = True


class CarBotChatTaskWorld(World):
    """
    Example one person world that talks to a provided agent (bot).
    """

    MAX_AGENTS = 1
    MODEL_KEY = 'tfidf_retriever'
    ...
    def generate_world(opt, agents):
        ...
        return CarBotChatTaskWorld(
            opt,
            agents[0],
            create_agent_from_shared(
                opt['shared_bot_params'][CarBotChatTaskWorld.MODEL_KEY]
            ),
        )
...
class CarOverworld(World):
    ...
    @staticmethod
    def generate_world(opt, agents):
        return CarOverworld(opt, agents[0])
    ...
    def parley(self):
        if self.first_time:
            self.agent.observe(
                {
                    'id': 'Overworld',
                    'text': 'Welcome to the overworld for the ParlAI Car '
